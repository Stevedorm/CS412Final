...
APPROX1="$SCRIPT_DIR/../cs412_tsp_approx.py"
APPROX2="$SCRIPT_DIR/../cs412_tsp_approx_partE.py"
...
if [[ ! -f "$APPROX2" ]]; then
  echo "ERROR: Cannot find cs412_tsp_approx_partE.py at $APPROX2" >&2
  exit 1
fi
...
RESULT_CSV="$SCRIPT_DIR/approx_vs_partE.csv"

# CSV header
echo "test_case,approx_result,approx_time,partE_result,partE_time" > "$RESULT_CSV"
...
" "${approx1_output}" | sed 's/$/\n/' | sed 's/^/    /'
  ...
  echo "-- cs412_tsp_approx_partE.py (approx 2) --"
...
echo "All done."
echo "Wrote combined approx vs partE results to: $RESULT_CSV"
