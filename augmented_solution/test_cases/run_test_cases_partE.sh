...
# run_test_cases.sh
#
# Runs cs412_tsp_approx.py on all test cases in the input/ directory and
...
#       with 12 vertices. Because cs412_tsp_approx.py uses a
#       approximate TSP algorithm that tries all permutations, this test case
...
# Path to cs412_tsp_approx_partE.py (one directory up)
SOL="$SCRIPT_DIR/../cs412_tsp_approx_partE.py"
...
if [[ ! -f "$SOL" ]]; then
  echo "ERROR: Cannot find cs412_tsp_approx.py at $SOL" >&2
  exit 1
fi
